...
async function run(inter) {
  ...
  const amount = inter.options.getInteger('amount');
	if(user.id===inter.user.id||user.bot){return   await inter.reply({ephemeral:true,embeds:[new MessageEmbed().setColor('#da9c83')
  ...
  if (amount <= interactor_amount) {
    ...
    .setTitle('/potato gift')
    .setAuthor({ name: inter.user.tag, iconURL: inter.user.displayAvatarURL()})
    .setDescription(`<@${user.id}> now has **${new_user_amount.toString()} potato${new_user_amount <= 1 ? "" : "es"}**! You now have **${new_interactor_amount.toString()} potato${new_interactor_amount <= 1 ? "" : "es"}**!`)
      .setThumbnail("https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/twitter/322/potato_1f954.png")
    .setTimestamp()
    .setFooter({ text: `in #${inter.channel.name}`});
