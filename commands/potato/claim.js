...
async function run(inter) {
  ...
  let filtered = messages.filter((message=>message.author.id==inter.client.user.id && message.content == ":potato:"))
  filtered.forEach(async (message)=>message.edit(`~~:potato:~~ - Claimed by <@${inter.user.id}>`).catch(err => {
                console.error(err);
            }))
